...
const API_KEY =
  "live_5QqqmF9zTBnZEvjuEfqzebdpCbC58e3XrKkNj9htkCtltg51XC6CJZCKRKtAFSjS";
 ...
 */
breedSelect.addEventListener("change", axiosGetImgOnSelect);

async function getImgOnSelect(e) {
...
async function axiosGetImgOnSelect(e){

  const response = await axios.get(
    `/images/search?limit=10&breed_ids=${e.target.value}`);
    axios.onDownloadProgress = updateProgess;
...
export async function favourite(imgId) {
  ...
  // const checkFave = await axios.get( `https://api.thecatapi.com/v1/favourites?image_id=${imgId}`)
  // if (checkFave.lengthComputable >1) { //already favorited
  //   const deleteImge = await axios.delete(`https://api.thecatapi.com/v1/favourites/${imgId}`)
  // }
...
async function getFavourites() {
  const checkFaves = await axios.get( '/favourites')
  const imgs = await checkFaves.data;
  console.log(imgs);
  repopulateCarousel(imgs);
}
/**
...
function repopulateCarousel(imgs) {
   // clear the carousel if it has any images
   if (document.getElementById("carouselInner").firstChild) {
    Carousel.clear();
  }
